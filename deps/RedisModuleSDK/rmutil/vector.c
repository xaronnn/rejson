...
POSIX_ONLY(inline) int __vector_PushPtr(Vector *v, void *elem) {
  if (v->top == v->cap) {
...
POSIX_ONLY(inline) int Vector_Get(Vector *v, size_t pos, void *ptr) {
...
POSIX_ONLY(inline) int Vector_Pop(Vector *v, void *ptr) {
  if (v->top > 0) {
    if (ptr != NULL) {
...
POSIX_ONLY(inline) int __vector_PutPtr(Vector *v, size_t pos, void *elem) {
...
POSIX_ONLY(inline) int Vector_Size(Vector *v) { return v->top; }

/* return the actual capacity */
POSIX_ONLY(inline) int Vector_Cap(Vector *v) { return v->cap; }
